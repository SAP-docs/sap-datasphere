...
Use the connection to access data from SAP ABAP on-premise systems through RFC or to access data from cloud source systems such as SAP S/4HANA Cloud through Web Socket RFC.



This topic contains the following sections:

-   [Supported Features](sap-abap-a75c1aa.md#loioa75c1aacf951449ba3b740c7e46da3a9__ABAP_usage)
-   [Configuring Connection Properties](sap-abap-a75c1aa.md#loioa75c1aacf951449ba3b740c7e46da3a9__connection_properties)

For information about the required prerequisites, see [Prepare Connectivity to SAP ABAP Systems](https://help.sap.com/viewer/935116dd7c324355803d4b85809cec97/internal/en-US/76c9ac1a318c4de2bea29e72c64be8a0.html "To be able to successfully validate and use a connection to an SAP ABAP system for remote tables or data flows, certain preparations have to be made.") :arrow_upper_right:.
...
-   Data from virtual tables through RFC for ODP sources \(extractors\):
    ...
    -   Extraction context *ABAP\_CDS* provides access to ABAP Core Data Services \(CDS\) Views that include the annotation `@Analytics.dataextraction.enabled: true` \(in SAP ABAP on-premise systems\)
    ...
    -   Extraction context *BW* provides access to InfoProviders in SAP BW or SAP BW∕4HANA systems:
        ...
        > ### Note:  
        > For accessing CompositeProviders and Queries in SAP BW∕4HANA we strongly recommended to use the model import with the specific connection type*SAP BW/4HANA Model Transfer*. For more information, see [SAP BW∕4HANA Model Transfer](sap-bw-4hana-model-transfer-1caba95.md).
...
-   When using *Web Socket RFC \(Use for S/4HANA Cloud Systems\)* endpoint, you can access all extraction-enabled ABAP CDS views that are C1-released that is views with annotation `@Analytics.dataextraction.enabled: true` that are available in the system. For more information, see [Data Integration](https://help.sap.com/docs/SAP_S4HANA_CLOUD/0f69f8fb28ac4bf48d2b57b9637e81fa/c3a5da91691d4ebd89748d9f40af7a4c.html) in the *SAP S/4HANA Cloud* documentation.
    ...
    > ### Note:  
    > To create new connections to SAP S/4HANA Cloud, we recommend using connection type *SAP S/4HANA Cloud* which supports both, view building and remote tables as well as data flows. For more information, see [SAP S/4HANA Cloud](sap-s-4hana-cloud-a98e5ff.md).
