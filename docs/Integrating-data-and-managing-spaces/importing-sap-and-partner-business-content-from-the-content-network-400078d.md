...
Users with an administrator role or space administrator role, can use the *Semantic Onboarding* app to import business content and sample content from SAP and partners published to the *Content Network*.



## Context

To import business content and sample content from SAP and partners via the *Semantic Onboarding* app, you must have either:
...
4.  Review the *Import Overview* section. For more details, and to modify any of the options, click the *Import Options* tab:
    ...
    > ### Note:  
    > With limited exceptions, object definitions only can be transported between SAP Datasphere tenants via the <span class="FPA-icons-V3"></span> \(*Transport*\) app. For information about including local table data in your package, see [Transporting Local Table Data](Transporting-Content-Between-Tenants/transporting-local-table-data-9ec1c3c.md).
    ...
    The import job status will be marked as complete only after both the import and deployment phases have successfully finished. Expect the import job to remain in progress until the deployment is fully completed.

    If you receive a deployment failure message after import, do not re-trigger the import job. Instead, manually deploy the objects directly to complete the process.
