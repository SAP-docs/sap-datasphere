...
6.  To manage other workload parameters, you can select either of the following in the *Configuration* dropdown list:
    ...
    -   *Custom*. These statement limit and admission control parameters are taken into account in the custom configuration. You can specify only the value for statements limits to set maximum total thread and memory limits that statements running concurrently in the space can consume:
        ...
        > ### Note:  
        > For resilience purposes, we limit to 80% the maximum CPU resources for workload generated by spaces, user group users and agent users. The remaining 20% of CPU resources are reserved to ensure that the system can respond under heavy load. If you choose a percentage up to 80%, it is applied. If you choose a percentage above 80% \(for example 90%\) the limit of 80% is applied.
        ...
        > ### Note:  
        > For resilience purposes, we limit to 80% the maximum memory resources reserved for workload generated by spaces, user group users and agent users. The remaining 20% of memory resources are reserved to ensure that the system can respond during high total memory consumption periods. If you choose a percentage up to 80%, it is applied. If you choose a percentage above 80% \(for example 90%\) the limit of 80% is applied.
        > 
        > In addition, when the total tenant memory consumption reaches 60%, any new statement in any space is limited to consuming 25% of the total tenant memory \(see [3372747](https://me.sap.com/notes/3372747)\). If the tenant memory usage is below 60%, then statements are limited to the total statement memory limit set for their space.
...
Data preview and deploy actions in the *Business Builder* and the *Data Builder*.
