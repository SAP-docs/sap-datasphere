...
You can obtain independent access to the underlying SAP HANA monitoring views that power the *Monitoring* apps to do additional analysis on them and visualize them in SAP Analytics Cloud.
...
> ### Note:  
> The data from these monitoring views is available directly in the *Monitoring* apps \(see [Monitoring SAP Datasphere](monitoring-sap-datasphere-d39b865.md)\). Working with them independently is optional and allows you to do further analysis that is not supported in the standard monitor.
...
-   `TASK_LOGS_V_EXT`:
    ...
    Duration in seconds of the task execution \(also works for ongoing execution\).
